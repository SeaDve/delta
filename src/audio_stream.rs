...
pub async fn receive(src_stream: Stream) -> Result<()> {
    let pipeline = gst::parse::launch(&format!(
        "giostreamsrc name={STREAMSRC_ELEMENT_NAME} ! matroskademux ! opusdec use-inband-fec=true ! audioconvert ! autoaudiosink",
    ))?
...
pub async fn transmit(sink_stream: Stream) -> Result<()> {
    let pipeline = gst::parse::launch(&format!(
        "pulsesrc name={PULSESRC_ELEMENT_NAME} ! audioconvert ! opusenc inband-fec=true ! matroskamux ! giostreamsink name={STREAMSINK_ELEMENT_NAME}",
    ))?
    .downcast::<gst::Pipeline>()
    .unwrap();
